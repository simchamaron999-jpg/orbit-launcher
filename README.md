...
| **Adaptive home layout** | Orbit calculates the central surface, ring radius, app-pod size, label behavior, and dock-safe space from the actual device window, controls, and app count. Static Small/Balanced/Large live-size presets are no longer used. |
| **Custom density** | Advanced users can select Custom layout density to make the adaptive composition modestly more compact or spacious without breaking safe geometry. |
| **App Library** | The All Apps dock action opens a searchable, adaptive square grid. It provides a full alphabetical app library, clear result counts, readable labels, and an empty state. |
| **Readable app density** | Pages can still use explicit visible-app targets of **8, 12, 16, 20, 24, or 32**. Dense rings reveal a full app name on hold instead of shrinking every label into overlap. |
...
| **Orbit icon treatment** | App artwork is displayed with adaptive Material surface pods, preserving installed-app icon identity while maintaining wallpaper-aware contrast. |
...
| **Supported BYOK provider** | OpenRouter only, using a user-entered API key and the configured free model. |
...
To use Ask Orbit, open **Settings → AI Engine** and enter your own OpenRouter API key. The app does not include any developer API key. OpenRouter receives the voice request and the installed-app list only as needed to resolve the request.
